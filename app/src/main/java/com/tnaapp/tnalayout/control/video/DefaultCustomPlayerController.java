 ...
 */
public final class DefaultCustomPlayerController extends FrameLayout implements CustomPlayerController, CustomVideoStateListener, CustomHandlerTouchRoot.OnTouchReceiver {
    ...
    private CustomPlayerControllerVisibilityListener visibilityListener;
    ...
    private View controlsRoot, bottomControlsRoot;
    private ProgressBar mProgress;
    private TextView mEndTime, mCurrentTime;

    private ImageButton mPauseButton, mNextButton, mPrevButton, mFullscreenButton;
    private ProgressBar loadingView;
    private int lastPlayedSeconds = -1;
    public boolean isFullScreenView;

    public DefaultCustomPlayerController(final Context context) {
        this(context, null);
    }

    public boolean setFullScreenView(boolean b) {
        return this.isFullScreenView = b;
    }

    public DefaultCustomPlayerController(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DefaultCustomPlayerController(final Context context, final AttributeSet attrs, final int defStyle) {
        super(context, attrs, defStyle);
    }

    @Override
    protected void onFinishInflate() {
        LayoutInflater.from(getContext()).inflate(R.layout.default_media_controller, this);
        initControllerView();
        super.onFinishInflate();
    }

    @Override
    public void setMediaPlayer(final CustomPlayer customPlayer) {
        mCustomPlayer = customPlayer;
        updatePausePlay();
    }

    public CustomPlayer getMediaPlayer() {
        return mCustomPlayer;
    }

    @Override
    public void setVisibilityListener(final CustomPlayerControllerVisibilityListener visibilityListener) {
    ...
    private void initControllerView() {
        ...
        CustomHandlerTouchRoot touchRoot = (CustomHandlerTouchRoot) findViewById(R.id.media_controller_touch_root);
        ...
        mFullscreenButton = (ImageButton) findViewById(R.id.media_controller_fullscreen);
        mFullscreenButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if (visibilityListener != null) {
                    visibilityListener.requestFullScreen();
                }
            }
        });
    }

    public void setFullSreenButtonIconByStage(boolean stage) {
        if (isFullScreenView || stage) {
            mFullscreenButton.setImageResource(R.drawable.ic_vidcontrol_fullscreen_on);
        } else {
            mFullscreenButton.setImageResource(R.drawable.ic_vidcontrol_fullscreen_off);
        }
    }
    ...
    private int setProgress() {
        if (mCustomPlayer == null || mDragging) {
            return 0;
        }
        int position = mCustomPlayer.getCurrentPosition();
        int duration = mCustomPlayer.getDuration();
        if (mProgress != null) {
            if (duration > 0) {
            ...
            int percent = mCustomPlayer.getBufferPercentage();
    ...
    public boolean dispatchKeyEvent(final KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HEADSETHOOK
            ...
            if (uniqueDown && !mCustomPlayer.isPlaying()) {
                mCustomPlayer.start();
                updatePausePlay();
                show(DEFAULT_TIMEOUT);
            }
    ...
    private void updatePausePlay() {
        ...
        if (mCustomPlayer.isPlaying()) {
            mPauseButton.setImageResource(R.drawable.ic_vidcontrol_pause);
        } else {
            mPauseButton.setImageResource(R.drawable.ic_vidcontrol_play);
        }
    }
    ...
    public void onInitializeAccessibilityEvent(final AccessibilityEvent event) {
        ...
        event.setClassName(DefaultCustomPlayerController.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(final AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(DefaultCustomPlayerController.class.getName());
    }
    ...
    private final SeekBar.OnSeekBarChangeListener mSeekListener = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(final SeekBar bar, final int progress, final boolean fromuser) {
            ...
            long duration = mCustomPlayer.getDuration();
            long newposition = (duration * progress) / 1000L;
            mCustomPlayer.seekTo((int) newposition);
            if (mCurrentTime != null) {
    ...
    private final Handler mHandler = new Handler() {
        ...
        public void handleMessage(final Message msg) {
            ...
            switch (msg.what) {
                ...
                case SHOW_PROGRESS:
                    pos = setProgress();
                    if (!mDragging && mShowing && mCustomPlayer.isPlaying()) {
